...
SpawnFruit:
    ...
    beq $s5, 3, SkipCyan # If last fruit cyan, skip clearing purple's position
    			 # i dont know why but i havae to use $s5 instead of $t0 here
...
AddLength:
li $s1, 1
# NEW CODE SEGMENT for unexpected change
...
jal isValidCoord        # check to see if the x and y values are valid coordinates
move $s5, $v0           # set the flag if the Top Left Pixel is a valid coord by moving the result to $s4
...
beqz $v0, skipBottomLeft  # skip drwing the top left pixel if its not a valid coordinate
jal CoordinateToAddress # convert the coordinate to its address
...
add $a0, $t1, $t3       # calculate the x value of the top left pixel
add $a1, $t2, $t5       # calculate the y value of the top right pixel
jal isValidCoord        # check to see if the x and y values are valid coordinates
move $s7, $v0           # set the flag if the Top Left Pixel is a valid coord by moving the result to $s4
beqz $v0, skipBottomRight  # skip drwing the top left pixel if its not a valid coordinate
jal CoordinateToAddress # convert the coordinate to its address
...
skipEraseTopLeft:
beqz $s5, skipEraseTopRight  # skips erasing the top left if the flag says its an invalid coordinate 
move $a0, $s1                # move the top left coordinate to $a0
jal DrawPixel                # draw that pixel with the background color 

skipEraseTopRight:
beqz $s6, skipEraseBottomLeft   # skips erasing the top left if the flag says its an invalid coordinate 
move $a0, $s2                   # move the top left coordinate to $a0
jal DrawPixel                   # draw that pixel with the background color 

skipEraseBottomLeft:
beqz $s7, skipEraseBottomRight   # skips erasing the top left if the flag says its an invalid coordinate 
move $a0, $s3                    # move the top left coordinate to $a0
jal DrawPixel                    # draw that pixel with the background color 
    
skipEraseBottomRight:
addi $t3, $t3, 1       # increment the offset by 1
